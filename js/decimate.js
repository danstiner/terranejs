// Adaptive terrain decimation: greedy Delaunay TIN to a vertical error
// tolerance. Vendored from Mapbox delatin (github.com/mapbox/delatin, ISC) so
// the pure-JS test suite can run it under node; the Delatin class below is
// verbatim. decimate() wraps it for our grids.

export function decimate(zGrid, width, height, maxError) {
  const d = new Delatin(zGrid, width, height);
  d.run(maxError);
  return { coords: d.coords, triangles: d.triangles }; // coords: [gx,gy,…]
}
...
class Delatin {
  ...
  run(maxError = 1) {
    while (this.getMaxError() > maxError) this.refine();
  }
  ...
  _legalize(a) {
    ...
    if (!inCircle(
      ...
      coords[2 * p1], coords[2 * p1 + 1])) return;
