...
import { prusaColorChangeXML, prusaFilamentConfig } from "./color-bands.js";
...
// PrusaSlicer project config carrying the starting filament colour. Real
// PrusaSlicer packages declare no content type for .config (or .xml) parts and
// read them by path, so no Content_Types entry is emitted for this one.
const CONFIG_PART = "Metadata/Slic3r_PE.config";
...
export class ThreeMFWriter {
  ...
  // Optional PrusaSlicer starting filament colour (#RRGGBB) for the base segment.
  setFilamentColor(hex) { this.filamentColor = hex || null; }
  ...
  async finish() {
    ...
    if (this.filamentColor) entries.push(entry(CONFIG_PART, prusaFilamentConfig(this.filamentColor)));
