...
import { BAND_COLORS, bandThresholds, bandOf, baseBand, colorChanges, prusaColorChangeXML, baseColorHex, prusaFilamentConfig }
...
test("prusaFilamentConfig: PrusaSlicer header + filament_colour line", () => {
  const cfg = prusaFilamentConfig("#295c8c");
  assert.match(cfg, /^; generated by PrusaSlicer/); // header must lead — gates loading
  assert.match(cfg, /; extruder_colour = ""/);      // empty so filament_colour wins
  assert.match(cfg, /; filament_colour = #295c8c\n$/);
});
